 ...
 */
public class SPARQLCacheAdapter extends SPARQLAdapter
{
    ...
    @Override
    public void add(String filenameOrURI, Model data)
    {
	add(filenameOrURI, data, null);
    }
	...
	super.add(graphUri, data);
	super.add(createGraphMetaModel(ResourceFactory.createResource(graphUri), filenameOrURI, endpoint));
    }

    @Override
    public boolean containsModel(String filenameOrURI)
    {
	Query query = QueryFactory.create("ASK { ?g <http://purl.org/net/opmv/ns#retrievedBy> _:x . _:x <http://purl.org/net/provenance/ns#accessedResource> <" + filenameOrURI + "> }");
	...
	request.setBasicAuthentication("2hwmvJSgCSV3fa3xGIb9", "X".toCharArray());
