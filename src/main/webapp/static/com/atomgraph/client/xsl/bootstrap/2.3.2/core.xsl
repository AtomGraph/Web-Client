...
exclude-result-prefixes="#all">
		...
		<div class="container-fluid">
                    ...
                    <div id="collapsing-top-navbar" class="nav-collapse collapse">
                        ...
                        <xsl:if test="$g:baseUri">
                            ...
                            <xsl:if test="key('resources-by-type', '&core;SPARQLEndpoint', document($g:baseUri))">
                                <ul class="nav pull-right">
                                    <xsl:apply-templates select="key('resources-by-type', '&core;SPARQLEndpoint', document($g:baseUri))" mode="bs2:NavBarListItem">
    ...
    <xsl:template match="*[*][@rdf:about] | *[*][@rdf:nodeID]" mode="xhtml:Title">
        ...
        <title>
            <xsl:if test="$g:baseUri">
                <xsl:apply-templates select="key('resources', $g:baseUri, document($g:baseUri))" mode="ac:label"/>
                <xsl:text> - </xsl:text>
            </xsl:if>
	...
	<script type="text/javascript" src="{resolve-uri('static/js/bootstrap.js', $ac:contextUri)}"></script>
        ...
        <xsl:if test="key('resources', $g:requestUri)/ac:mode/@rdf:resource = '&ac;MapMode' or key('resources', $g:requestUri)/ac:forClass/@rdf:resource">
    ...
    <xsl:template match="rdf:RDF[key('resources', $g:requestUri)/ac:forClass]" mode="bs2:ModeList" priority="1"/>
    -->
    ...
    <xsl:template match="*[@rdf:about]" mode="bs2:Actions" priority="1">
        ...
        <xsl:if test="key('resources-by-layout-of', key('resources', $g:requestUri)/ac:layoutOf/@rdf:resource)[ac:mode/@rdf:resource = '&ac;EditMode']/@rdf:about and not(key('resources', $g:requestUri)/ac:mode/@rdf:resource = '&ac;EditMode')">
            <div class="pull-right">
                <a class="btn btn-primary" href="{key('resources-by-layout-of', key('resources', $g:requestUri)/ac:layoutOf/@rdf:resource)[ac:mode/@rdf:resource = '&ac;EditMode']/@rdf:about}">
    ...
    <xsl:template match="*[@rdf:about]" mode="bs2:Form">
        <xsl:param name="method" select="'post'" as="xs:string"/>
        <xsl:param name="action" select="xs:anyURI(concat($g:absolutePath, '?_method=PUT&amp;mode=', encode-for-uri('&ac;EditMode')))" as="xs:anyURI"/>
